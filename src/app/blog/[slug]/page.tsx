    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 border border-slate-200 text-center max-w-md w-full shadow-sm">
          ...
          <Link
            href="/blog"
            className="inline-block bg-slate-900 hover:bg-blue-600 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors"
          >
            ← 매거진 목록으로
          </Link>
        </div>
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 flex flex-col font-sans">
      ...
      <div className="bg-white border-b border-slate-100 py-3 px-4 text-xs text-slate-500">
        <div className="max-w-4xl mx-auto flex items-center gap-2">
          ...
          <span>&gt;</span>
          <Link href="/blog" className="hover:text-blue-600">혜택 매거진</Link>
          <span>&gt;</span>
      ...
      <main className="max-w-4xl w-full mx-auto px-4 py-10 flex-1 space-y-10">
        <article className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/90 shadow-sm space-y-6">
            ...
            {post.tags && post.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {post.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-xs text-slate-500 bg-slate-100 px-2.5 py-1 rounded-lg"
                  >
                    #{tag}
                  </span>
                ))}
          ...
          <div className="bg-blue-50/70 border border-blue-100 rounded-2xl p-4 text-xs text-blue-900 leading-relaxed space-y-1">
            ...
            <p>
              본 글은 공공데이터포털(data.go.kr)에 등록된 공식 정책 자료를 바탕으로 알기 쉽게 재구성한 생활 정보입니다. 주관 기관의 사정 및 예산 소진 상황에 따라 신청 조건이 변동될 수 있으니 신청 전 해당 기관의 최종 공고문을 꼭 확인해 주세요.
            </p>
          </div>
          ...
          <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
            <Link
              href="/blog"
              className="inline-flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-5 py-2.5 rounded-xl text-xs sm:text-sm transition-colors"
            >
              ← 목록으로 돌아가기
            </Link>
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold px-5 py-2.5 rounded-xl text-xs sm:text-sm transition-colors shadow-sm"
            >
              홈(종합 정보)으로 이동
            </Link>
          </div>
        ...
        {/* 4. 구글 애드센스 체류 시간 증가를 위한 [함께 보면 좋은 다른 이야기] 추천 섹션 */}
        {relatedPosts.length > 0 && (
          <section className="space-y-4">
            <h2 className="text-xl font-black text-slate-900 flex items-center gap-2">
              <span className="text-blue-600">📖</span> 함께 읽으면 도움되는 이야기
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {relatedPosts.map((related) => (
                <Link
                  ...
                  className="bg-white p-5 rounded-2xl border border-slate-200/90 shadow-sm hover:shadow-md hover:border-blue-300 transition-all flex flex-col justify-between group"
                >
                  ...
                  <span className="text-xs text-blue-600 font-medium pt-3 mt-2 border-t border-slate-50 flex items-center gap-1">
                    읽어보기 &rarr;
                  </span>
                </Link>
              ))}
